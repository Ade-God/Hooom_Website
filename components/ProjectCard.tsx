  ...
  return (
    ...
    >
      <div className="flex items-center justify-between gap-2">
        <p className="text-xs uppercase tracking-[0.2em] text-brand-orange">{project.industry}</p>
        <span className="text-xs text-slate-500">{project.tags.join(" • ")}</span>
      </div>
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        {project.servicesProvided.map((service) => (
          <span key={service} className="rounded-full bg-slate-100 text-slate-700 px-3 py-1 text-xs font-semibold">
            {service}
          </span>
        ))}
      </div>
